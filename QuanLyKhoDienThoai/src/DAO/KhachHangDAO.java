...
public class KhachHangDAO implements DAOinterface<KhachHangDTO> {
    ...
    public int insert(KhachHangDTO t) {
        ...
        try {
            ...
            String sql = "INSERT INTO `khachhang`(`makh`, `tenkhachhang`, `diachi`,`sdt`, `email`, `trangthai`) VALUES (?,?,?,?,?,1)";
    ...
    public int update(KhachHangDTO t) {
        ...
        try {
            Connection con = (Connection) JDBCUtil.getConnection();
            String sql = "UPDATE `khachhang` SET `makh`=?,`tenkhachhang`=?,`diachi`=?,`sdt`=?,`email`=? WHERE makh=?";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
            pst.setInt(1, t.getMaKH());
            pst.setString(2, t.getHoten());
            pst.setString(3, t.getDiachi());
    ...
    public ArrayList<KhachHangDTO> selectAll() {
        ...
        try {
            ...
            while(rs.next()){
                ...
                KhachHangDTO kh = new KhachHangDTO(makh, tenkhachhang, sdt, diachi, email, ngaythamgia);
                result.add(kh);
            }
            JDBCUtil.closeConnection(con);
        } catch (Exception e) {
    ...
    public KhachHangDTO selectById(String t) {
        ...
        try {
            ...
            while(rs.next()){
                ...
                String email = rs.getString("email");
                Date ngaythamgia = rs.getDate("ngaythamgia");
                result = new KhachHangDTO(makh, tenkhachhang, sdt, diachi, email, ngaythamgia);
            }
            JDBCUtil.closeConnection(con);
        } catch (Exception e) {
            System.out.println(e);
